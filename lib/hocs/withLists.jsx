...
export default function ListsHOC(WrappedComponent) {
  return class withListData extends Component {
      ...
      if(this.props.usePages || this.props.useLimit) {
        ...
        if(this.props.useLimit) {
          return this.props.fetch({
            ...this.props.filters, offset: offset + limit, limit: 25, ...this.props.apiParams,
          })
        }
      }
    }
    ...
    getInitialData = () => {
      if(this.props.usePages) {
        return this.props.fetch({ ...this.props.filters, page: 0, ...this.props.apiParams }, { dataFunction: 'replace' })
      }
      if(this.props.useLimit) {
        return this.props.fetch({
          ...this.props.filters, offset: 0, limit: 25, ...this.props.apiParams,
        }, { dataFunction: 'replace' })
      }
      return this.props.fetch({ ...this.props.filters, ...this.props.apiParams }, { dataFunction: 'replace' })
    }
    ...
    addFilter = (filter) => {
      ...
      if(this.props.useLimit) {
        return this.props.fetch({
          ...this.props.filters, offset: 0, limit: 25, ...this.props.apiParams, ...filter,
        }, { dataFunction: 'replace' })
      }
      return this.props.fetch({ ...this.props.filters, ...this.props.apiParams, ...filter }, { dataFunction: 'replace' })
    }
